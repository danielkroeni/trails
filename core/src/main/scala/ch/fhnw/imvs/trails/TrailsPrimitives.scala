...
final case class State[+Head <: SchemaElement](path: List[Elem], cycles: Set[List[Elem]], labels: Map[String,List[List[Elem]]])
...
def label[I<: SchemaElement,O<: SchemaElement,A](tr: Tr[Env,State[I],State[O],A], name: String): Tr[Env,State[I],State[O],A] = {
  for {
    (sl,a) <- slice(tr)
    _      <- updateState[Env,State[O],State[O]]((s: State[O]) => s.copy(labels = s.labels.updated(name, sl :: s.labels(name)) ))
  } yield a
}

def label[S<: SchemaElement,A](name: String): Tr[Env, State[S],State[S],List[List[Elem]]] =
  getState.map(_.labels(name))

def sub[S <: SchemaElement, O<: SchemaElement,A](tr: Tr[Env,State[S],State[O],A]): Tr[Env,State[S],State[S],A] =
  Tr(e => s => tr(e)(s).map{ case (_, a) => (s, a) })
}
